$(function() {
	...
	$('#editor-workflow').workflow({
		...
		propertyChangeCommand : function(event, ui) {// 监听属性发生变化的函数
			$('body', document).application('executeCommand', 'setProperty',
					$.extend({
								event : event
							}, ui));
		},
		moveCommand : function(event, ui) {
			$('body', document).application('executeCommand', 'move',
					$.extend({
								event : event
							}, ui));
		},// 集成鼠标动作的move命令
		addTransitionCommand : function(event, ui) {
			$('body', document).application('executeCommand', 'addTransition',
					$.extend({
								event : event
							}, ui));
		},// 集成鼠标动作的addTransition命令
		removeTransitionCommand : function(event, ui) {
			$('body', document).application('executeCommand',
					'removeTransition', $.extend({
								event : event
							}, ui));
		}
	});
	...
	$('#application-layout').layout({
				...
				onResize : function(width, height) {
					$('#editor-workflow,#editor-workflow .bg').css({
								width : (width - 6),
								height : (height - 6)
							});
	...
	$('body', document).application({
		// 所有command执行后，更新工具栏的undo、redo以及save按钮的显示
		afterCommand : function() {
			var commands = $(this).application('getCommands'), undoCommands = $(this)
					.application('getUndoCommands'), actives = [], disables = [];
			...
			.application('registerCommand', 'addNode', {
				...
				exec : function() {
					// 执行的动作
					var ui;
					ui = $('#editor-workflow').workflow('addNode',
							this.options.id, this.options.text,
							this.options.type, this.options.offset);
					if (!ui) {
				...
				undo : function() {
					// 撤销的动作
					$('#editor-workflow').workflow('removeNode',
							this.options.id);
					// 删除树中的节点
					$('#tree-workflow-element').tree('removeNode',
							this.options.id);
				},
				defaults : {
					text : '新增',
				...
				exec : function(isRedo) {
					...
					if (o.event) {// 事件驱动的动作,不重复执行操作。
						...
						if (o.selectorPath[0] == '.ui-selected') {//
							var selectorPath = [], moveElements = [];
							$('#editor-workflow')
									.find(o.selectorPath.join(' ')).each(
											function() {
												moveElements
														.push('#'
																+ this
						...
						$('#editor-workflow').workflow('move', o.selectorPath,
								o.position, o.originalPosition);
					}
				},
				undo : function() {
					var o = this.options;
				...
				exec : function() {
					...
					if (o.event) {// 事件驱动的动作,不重复执行操作。
						...
						this.options.id = $('#editor-workflow').workflow(
								'addTransition', o.fromId, o.toId);
					}
					...
					$('#tree-workflow-element').tree('addNodeChild',
							'transition', {
								id : transitionId,
								text : transitionId,
				...
				undo : function() {
					var o = this.options;
					$('#editor-workflow').workflow('removeTransition',
							o.fromId, o.toId);

					var transitionId = o.fromId + '-' + o.toId;
					$('#tree-workflow-element')
							.tree('removeNode', transitionId);
				},
				defaults : {
					text : '新增'
				}
				...
				exec : function() {
					...
					if (o.event) {// 事件驱动的动作,不重复执行操作。
						...
						$('#editor-workflow').workflow('setProperty',
								o.selectorPath, o.property, o.value);
					}
				},
				undo : function() {
					var o = this.options;
					$('#editor-workflow').workflow('setProperty',
							o.selectorPath, o.property, o.oldValue);
				}
			}).application('registerCommand', 'removeNode', {
				getTitle : function() {
					return 'REMOVE NODE:' + this.options.type;
				},
				exec : function() {
					$('#editor-workflow').workflow('removeNode',
							this.options.id);
			...
			}).application('registerCommand', 'removeTransition', {
				getTitle : function() {
					return '删除连线';
				},
				exec : function() {
	...
	$('#application-palette').palette({
		drop : function(event, ui) {
			var offset = {
				left : event.pageX - ui.drop.offset().left
						+ ui.drop.scrollLeft(),
				top : event.pageY - ui.drop.offset().top + ui.drop.scrollTop()
			};
			$('body', document).application('executeCommand', 'addNode', {
						text : ui.text,
						offset : offset,
						type : ui.type
					});
	...
	function initWorkElementsTree() {
		...
		$('#editor-workflow .node').each(function() {
			...
			nodesHtmls.push('<li class="treeNode '
					+ this.className.split(' ')[1] + '" id="' + id
					+ '"><span><a href="#">' + ($(this).text() || id)
					+ '</a></span></li>');
		});
		nodesHtmls.push('</ul>');
		...
		$('#editor-workflow .transition').each(function() {
			var id = this.getAttribute('id');
			transHtmls.push('<li class="treeNode transition" id="' + id
					+ '"><span><a href="#">' + (id) + '</a></span></li>');
		});
		transHtmls.push('</ul>');
