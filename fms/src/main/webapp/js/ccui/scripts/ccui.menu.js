	...
	$.widget("ccui.menu", {
		...
		_initMenu : function() {
			...
			this.element.bind('click', function(event) {
				...
				if (clickLi.hasClass('v-menuBar')) {// 垂直菜单栏
					...
					//
					var command = clickLi.attr('command'), action = clickLi
							.attr('action'), checkOk;
				...
				if (overLi.hasClass('menuBar')) {
					var openLi = $('.menuBar.expanded', this)
							.removeClass('expanded');
					if (openLi.length > 0) {
					...
					overLi.parent().find('>li.expanded').not(overLi)
							.removeClass('expanded');
					if (overLi.hasClass('expandable')) {
						overLi.addClass('expanded');
						var top = overLi.offset().top
								- overLi.offsetParent().offset().top - 5, left = overLi
								.offset().left
								+ overLi.width()
								- overLi.offsetParent().offset().left - 5;
						overLi.find('>ul').css({
									left : left,
									top : top
								});
		...
		active : function(actives) {
			...
			for (var i = 0; i < actives.length; i++) {
				this.element.find('li.disabled#' + actives[i])
		...
		disable : function(disables) {
			...
			for (var i = 0; i < disables.length; i++) {
				this.element.find('li.menuItem#' + disables[i])
						.addClass('disabled');
			}
		}
	});
