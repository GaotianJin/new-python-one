	...
	$.ccui.inputField = $.extend({}, $.ccui.field, {
		_initField : function() {
			...
			this.element.width(this.options.width).html('<input '
					+ (this.options.readonly ? 'readonly="readonly"' : '')
					+ ' id="v-'
					+ this.options.id
					+ '" name="'
					+ this.options.property
		...
		validate : function() {
			...
			var a = this.getValue();
			if (this.options.minLength && a.length < this.options.minLength) {
				this._validateError('');
				return false
			}
			if (this.options.maxLength && a.length > this.options.maxLength) {
				this._validateError('');
				return false
			}
			var b;
		...
		fieldBlur : function() {
			...
			window.setTimeout(function() {
				if (o.readonly != true && o.checkUrl
						...
						&& !p.element.is('.validating-fail') && p.validate()) {
					var c = p.getValue(), src = $.ccui.addUrlParameter(
							o.checkUrl, 'property:' + o.property, c);
		...
		_validatingPass : function(a) {
			this.element.removeClass('validating')
					.removeClass('validating-fail').addClass('validating-pass')
					.attr('title', a)
		},
		_validatingFail : function(a) {
			this.element.removeClass('validating')
					.removeClass('validate-success')
					.removeClass('validating-pass').addClass('validating-fail')
					.attr('title', a)
		},
	...
	$.widget("ccui.tree", $.extend({}, $.ui.mouse, {
		...
		addNodeChild : function(a, b) {
			...
			if (c.length > 0 && this.element.find('#' + b.id).length == 0) {
				...
				c.addClass('expandable').addClass('expanded');
			}
		},
...
(function($) {
	$.widget("ccui.layout", {
		...
		_generateSize : function() {
			...
			this.element.find('>#split-north').css('top', northHeight - 1);
