...
function detail(){
	...
	if(row.taskStatus=='理赔'){
		if(row.activityName=="初审中"){
			parent.addtab('理赔初审','claim/claimCheckOrApproveUrl?id='+row.dbid+"&flag=noOper");
		}
	
		if(row.activityName=="复核中"){
			parent.addtab('理赔复核','claim/claimCheckOrApproveUrl?id='+row.dbid+"&flag=noOper");
		}
	}
}
...
function toDo(){
	...
	if(row.taskStatus=='保全'){
		if(row.activityName=="处理中")
			parent.addtab('保全处理','pos/getPosPageUrl?id='+row.BussNo+"&flag=Oper");
			if(row.activityName=="复核中")
			parent.addtab('保全复核','pos/getPosPageUrl?id='+row.BussNo+"&flag=Oper");
	}
	if(row.taskStatus=='理赔'){
		if(row.activityName=="初审中"){
...
/** 获取保全复核任务 */
function getBQCheck(){
	$.post("../pos/getCheckTaskUrl",  function(data) {
		if(data.msg=="success"){
			var id = data.id;
			parent.addtab('保全复核','pos/getPosPageUrl?id='+id+"&flag=Oper");
		}else{
...
/** 申请保全 */
function applyBQ(){
	parent.addtab('保全申请','pos/dealPosUrl');
}
