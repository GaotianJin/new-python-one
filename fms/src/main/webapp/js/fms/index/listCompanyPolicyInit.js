...
jQuery(function($) {
	...
	roleId = $("#comPolicy_roleId").val();
	//comPolicyRoleControl();
})

/**
 * 权限控制
 */
function comPolicyRoleControl(){
	try{
		if(roleId !=null){
			if(roleId!='1'){
				$("#comPolicy_linkbutton").hide();
			}
		}else{
 ...
 * 公司政策列表
 */
function initListCompanyPolicyTable(){
	var  selectIndex = -1;
	customerTable = $('#listCompanyPolicyTable').datagrid({
		title : '公司政策列表', // 标题
		method : 'post',
		...
		columns : [[
				{
					...
					return "<a href = '#' onclick=detailPolicytab('公司政策详情','"
		...
		onLoadSuccess : function() {
			$('#listCompanyPolicyTable').datagrid('clearSelections'); // 一定要加上这一句，要不然datagrid会记住之前的选择状态，删除时会出问题
		}
...
function detailPolicytab(title, href) {
	$('<div id="addCompanyPolicyWindow"/>').window({
		href : href,
		modal : true,
		title : title,
		fit : true,
