...
function initListCustomerTable(){
	var  selectIndex = -1;
	customerTable = $('#listCustomer_CustomerTable').datagrid({
		title : '投资查询列表', // 标题
		method : 'post',
		...
		columns : [[{
				...
				},
				{
					...
					formatter : function(value, row, index) {
						...
						param.customerNo= row.customerNo;
						param = $.toJSON(param);
//						return "<a href='#'  onclick=addcustomeWindow('客户明细信息','"+contextPath+"/customer/modifyCustomerInfoUrl?param="+param+"')>"+row.customerNo+"</a>";
						return "<a href='#'  onclick=addcustomeWindow('投资明细信息','"+contextPath+"/modifyCustomer/modifyCustomerInvestInfoUrl?param="+param+"')>"+row.customerNo+"</a>";
					} // 需要formatter一下才能显示正确的数据
				},
				{
					field : 'chnName',
...
//						return row.chnName;
						...
						param.customerNo= row.customerNo;
						param = $.toJSON(param);
						return "<a href='#'  onclick=addcustomeWindow('投资明细信息','"+contextPath+"/modifyCustomer/modifyCustomerInvestInfoUrl?param="+param+"')>"+row.chnName+"</a>";
					} // 需要formatter一下才能显示正确的数据
				},
				/*{
					field : 'lastName',
...
//				}, '-'*/],
		onLoadSuccess : function() {
			$('#listCustomer_CustomerTable').datagrid('clearSelections'); // 一定要加上这一句，要不然datagrid会记住之前的选择状态，删除时会出问题
		}
...
function initListCustomerCombobox(){
	...
	//初始化机构
	$("#lisCustomer_comId").combobox({
		url : contextPath + '/codeQuery/comQuery',
		valueField : 'comId',
		textField : 'comName'
	});
	//初始化团队
	$("#lisCustomer_DepartmentId").combobox({
		valueField : 'departmentId',
		textField : 'departmentName',
		onShowPanel : function(){
			var comId = $("#lisCustomer_comId").combobox("getValue");
	...
	$("#lisCustomer_AgentId").combobox({
		...
		onShowPanel : function(){
			...
			var departmentId = $("#lisCustomer_DepartmentId").combobox("getValue");
	...
	//客户级别
	$('#lisCustomer_custLevel').combobox({
		valueField:'code',    
	    textField:'codeName',  
	...
	//是否国金开户
	$('#ListCustomer_isGoldenStateopen').combobox({
		valueField:'code',    
	    textField:'codeName',  
	    url:'codeQuery/tdCodeQuery?codeType=isGoldenStateopen'
	});
	//出生月份
	$('#ListCustomer_birthMonth').combobox({
		valueField: 'code',
		textField: 'codeName',
...
function queryCustomerList() {
	...
	var queryParam = $("#listCustomer_queryConditionForm").serialize();
...
// 清空查询条件然后进行查询
function clearForm() {
	$('#listCustomer_queryConditionForm').form('clear');
	queryCustomerList();
}
...
function exportCustBaseInfo() {
	var queryParam = $("#listCustomer_queryConditionForm").serialize();// 获取用户表单中的输入信息
