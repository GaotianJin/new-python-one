jQuery(function($) {
	...
	/*if(1!=userId){
		$("#listCustPhone_button").hide();
	}*/
});


var customerPhoneTable ;
function initListCustomerTable(){
	var  selectIndex = -1;
	customerPhoneTable = $('#listCustPhone_CustomerTable').datagrid({
		//title : '客户列表', // 标题
		method : 'post',
		...
		url : contextPath+"/customer/queryAllCustomerList",
		sortName : 'id', // 排序的列
		sortOrder : 'desc', // 倒序
		...
		onLoadSuccess : function() {
			$('#listCustPhone_CustomerTable').datagrid('clearSelections'); // 一定要加上这一句，要不然datagrid会记住之前的选择状态，删除时会出问题
		}
...
function initListCustomerCombobox(){
	...
	//初始化机构
	$("#lisCustPhone_comId").combobox({
		url : contextPath + '/codeQuery/comQuery',
		valueField : 'comId',
		textField : 'comName'
	});
	//初始化团队
	$("#lisCustPhone_DepartmentId").combobox({
		valueField : 'departmentId',
		textField : 'departmentName',
		onShowPanel : function(){
			var comId = $("#lisCustPhone_comId").combobox("getValue");
	...
	$("#lisCustPhone_AgentId").combobox({
		...
		onShowPanel : function(){
			...
			var departmentId = $("#lisCustPhone_DepartmentId").combobox("getValue");
	...
	//客户级别
	$('#lisCustPhone_custLevel').combobox({
		valueField:'code',    
	    textField:'codeName',  
	...
	//客户类型
	$('#lisCustPhone_custType').combobox({
		valueField:'code',    
	    textField:'codeName',  
	...
	//是否国金开户
	$('#ListCustPhone_isGoldenStateopen').combobox({
		valueField:'code',    
	    textField:'codeName',  
	    url:'codeQuery/tdCodeQuery?codeType=isGoldenStateopen'
	});
	//出生月份
	$('#ListCustPhone_birthMonth').combobox({
		valueField: 'code',
		textField: 'codeName',
...
// 表格查询
function queryCustPhoneList() {
	/*var params = $('#listCustPhone_CustomerTable').datagrid('options').queryParams; // 先取得
	var fields = $('#listCust_queryConditionForm').serializeArray(); // 自动序列化表单元素为JSON对象
	$.each(fields, function(i, field) {
				params[field.name] = field.value; // 设置查询参数
			});
	$('#listCustPhone_CustomerTable').datagrid('cisreload'); // 设置好查询参数 reload 一下就可以了*/
	/*customerPhoneTable.datagrid('options').url = contextPath+"/customer/queryCustomerList";*/
	customerPhoneTable.datagrid('options').url = contextPath+"/customer/queryAllCustomerList";
	var queryParam = $("#listCustPhone_queryConditionForm").serialize();
	...
	customerPhoneTable.datagrid('load',{queryParam:queryParam});	
...
// 清空查询条件然后进行查询
function clearCustPhoneForm() {
	$('#listCustPhone_queryConditionForm').form('clear');
	queryCustPhoneList();
}
 ...
 * 客户手机号数据导出
 */
function exportCustPhoneInfo() {
	var queryParam = $("#listCustPhone_queryConditionForm").serialize();// 获取用户表单中的输入信息
		queryParam = formDataToJsonStr(queryParam);// 用户表单中信息转换为JSON格式字符串
		window.open(contextPath
				+ '/customer/custPhoneInfoDetail.xls?queryParam='
