...
//var customerNo = null;
jQuery(function($) {
	productId = $("#detail_productInvestMentId").val();
	productSubType = $("#detail_productInvestMentSubType").val();
//	customerNo = $("#investMentCustomercustomerNoId").val();
	if(productSubType == '01'){
		$(".hideInfo1").remove();
		$(".hideInfo2").remove();
		$(".investStrategy").remove();
		$(".investScope").remove();
		$('#incomeDisInvestMentInfo').hide();
		$('#fixedDiffrentInvestMentInfo').hide();
		$('#feeInvestMentInfo').hide();
		$('#diffrentInvestMentInfo').hide();
		$("#productNetValueInvestMentInfo").hide();
	}
		...
		success : function (result){
			try{
				if(result.success){
					...
					setInputValueById("detailProductInvestMentDetailForm",resultObj);
				}else{
...
//产品净值信息查看
function productNetValueInvestMentInfo(){
	var param = {};
	param.productId = productId;
	param.productSubType = productSubType;
...
var detialWealthIncomeDisInvestMentInfoTable;
function initWealthIncomeDisInfoTable(){
	detialWealthIncomeDisInvestMentInfoTable = $("#detialWealthIncomeDisInvestMentInfoTable").datagrid({
		method : 'post',
 ...
 * 
 */
var fixedWealthRateInvestMentTable;
function initWealthRateTable() {
	fixedWealthRateInvestMentTable = $("#fixedWealthRateInvestMentTable")
			.datagrid(
					{
						method : 'post',
						iconCls : 'icon-edit', // 图标
						...
						onLoadSuccess : function() {
							$('#fixedWealthRateInvestMentTable').datagrid('clearSelections'); // 一定要加上这一句，要不然datagrid会记住之前的选择状态，删除时会出问题
						}
					});
}
...
function getUpdateInitValue(){
	$.ajax({
		...
		data:{param:$('#detail_productInvestMentId').val()},
		cache:false,
		success:function(result){
				if(result.success){
					var resultObj = result.obj;
					if (resultObj.modifyPdWealthChargeRate!=null&&resultObj.modifyPdWealthChargeRate!=undefined&&resultObj.modifyPdWealthChargeRate!="") {
						//固定收益分类信息
						setInputValueById("fixedDiffrentInvestMentInfoForm",resultObj.modifyPdWealthChargeRate);
						if(resultObj.modifyPdWealthChargeRate2!=null && resultObj.modifyPdWealthChargeRate2!=" "){
						$("#productDetail_taxFeeInvestMent").val(resultObj.modifyPdWealthChargeRate2.taxFee);
						$("#productDetail_channelFeeInvestMent").val(resultObj.modifyPdWealthChargeRate2.channelFee);}
					}
					if (resultObj.modifyPdWealthFeeRateList!=null&&resultObj.modifyPdWealthFeeRateList!=undefined&&resultObj.modifyPdWealthFeeRateList!="") {
						//财富费用比例信息
						loadJsonObjData("fixedWealthRateInvestMentTable",resultObj.modifyPdWealthFeeRateList);
					}
					...
					// 财富固定收益类收益分配信息
					loadJsonObjData("detialWealthIncomeDisInvestMentInfoTable",resultObj.modifyPdwealthIncomeDisMapList);
					}
					}else{
						$.messager.alert('提示', result.msg);
					}
