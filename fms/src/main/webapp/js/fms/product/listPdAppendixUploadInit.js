...
function initAllCombobox() {
	...
	// 产品初始化 显示为：产品代码-产品名称
	productIdComobox = $("#pdauProductId").combobox({
	...
	agencyComIdCombobox = $("#pdauAgencyComId").combobox(
			{
	...
	productSubTypeCombobox = $("#pdauProductSubType").combobox({
		valueField : 'code',
		textField : 'codeName'
	});
	// 产品类型初始化
	var productTypeCombobox;
	productTypeCombobox = $("#pdauProductType").combobox(
			{
				url : contextPath
...
// 产品设置页面信息列表初始化
var initListPdTable;
function initListPdTable() {
	var  selectIndex = -1;
	initListPdTable = $('#listPdAppendixTable')
			.datagrid(
					{
						title : '产品信息列表', // 标题
						method : 'post',
						...
						toolbar : [
								{
									...
									iconCls : 'icon-add',
									handler : function() {
										var rows = $('#listPdAppendixTable').datagrid('getSelections');
										if (rows.length == 0) {
										...
										param.flag = "addFile";
										addPdFileWindow('文件上传', contextPath + "/fileUpload/fileUploadUrl?param="+ $.toJSON(param));
									}
								},'-'],
						onLoadSuccess : function() {
							$('#listPdAppendixTable').datagrid('clearSelections'); 
						},
...
function searchproductInfo() {
	...
	initListPdTable.datagrid('options').url = contextPath+"/product/queryListProductUrl";
	var queryparam = $("#listPdForm").serialize();
	queryparam = formDataToJsonStr(queryparam);
	initListPdTable.datagrid('load',{param:queryparam});
	
}

//清空查询条件然后进行查询
function clearSearchInfo() {
	$('#listPdForm').form('clear');
	searchproductInfo();
}

//上传文件
function addPdFileWindow(title, href) {
