...
var customerNo= $("#investCustomercustomerNoId").val();
jQuery(function($) {
	//产品查询时显示该产品的所有净值走势情况
	getPdSearchNetValue();
...
function getPdSearchNetValue(){
	...
	$.ajax({
		type:'post',
		url:contextPath + "/product/getPdInvestSearchNetValue",
...
function initNetValueDetailId() {
	var  selectIndex = -1;
	pdNetValueDetailId = $("#pdnetInvestValueDetailId")
			.datagrid(
					{
						title : '净值信息列表', // 标题
						method : 'post',
						...
						onLoadSuccess : function() {
							$('#pdnetInvestValueDetailId').datagrid('clearSelections'); // 一定要加上这一句，要不然datagrid会记住之前的选择状态，删除时会出问题
						},
...
function initPdSearchNetVauleChart(resultObj){
	$('#pdnetInvestValueInfo_DetailFormDiv').highcharts({ 
