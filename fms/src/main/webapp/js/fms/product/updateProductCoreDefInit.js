...
function initAllValidateBox(){
	...
	$('#modify_subscriptionFeeRatio').validatebox({required:true});
	//所有金额输入框，输入时转换大写
				...
				if(result.success){
					...
					if(modifyProductTypeCode=='1'){
						...
						if(resultObj.modifyPdWealth.expectOpenDayRules!=null&&resultObj.modifyPdWealth.expectOpenDayRules!=undefined&&resultObj.modifyPdWealth.expectOpenDayRules!=""){
							...
							$('#expectOpenDayRules').combobox('setValues', a);
						}
						}
						//01-股权,02-固定收益类，03-浮动收益
						if(modifyProductSubTypeCode=="02"){
							...
							$("#wealthStockDisInfo").parent(".panel").css("display","none");//隐藏股权类收益分配信息
							...
							$("#promptIncomeWayTd").remove();//移除收益方式单元格
							$("#floatProductInfo").append('<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>');
...
function initAllCombobox() {
	...
	if(modifyProductTypeCode=='1'){
		...
		if(modifyProductSubTypeCode=='03'||modifyProductSubTypeCode=="04"){
			...
			//产品开放日规则
			expectOpenDayRulesCombobox = $("#expectOpenDayRules").combobox({
	...
	if (modifyProductTypeCode=='1') {
		if (modifyProductSubTypeCode=='02') {
			...
			$('#modify_closeDperiodUnit').combobox({required:true});//封闭期单位非必填
		} else {
			// 收益方式
			$("#modifyPdCoreInfo_incomeWay").combobox({
				url : contextPath + '/codeQuery/tdCodeQuery?codeType=incomeWay',
			...
			//$('#modifyPdCoreInfo_historyEarnrate').validatebox({required:false});
			$('#modifyPdCoreInfo_historyEarnrate').validatebox({required:false});
			$('#modify_closeDperiod').validatebox({required:false});//封闭期非必填
			$('#modify_closeDperiodUnit').combobox({required:false});//封闭期单位非必填
		}
	}
...
function submitUpdateProductInfo() {
	...
	if (productSubTypeValue == "03" || productSubTypeValue == "04") {
		...
		var expectOpenDayRulesText= $("#expectOpenDayRules").combobox('getText');
		$("#expectOpenDayRules").combobox('setValue',expectOpenDayRulesValue);
		$("#expectOpenDayRules").combobox('setText',expectOpenDayRulesText);
	}
	...
	if (productTypeValue == 2) {
		//获取保险产品基本信息
		var insuranceInfoJson = formDataToJsonObj($("#insuranceInfoForm").serialize());
		...
		//获取财富产品的基本信息
		/*var wealthProductInfoJson = formDataToJsonObj($("#wealthProductInfoForm").serialize());
		param.wealthProductInfo = wealthProductInfoJson;*/
		
		wealthProductBaseInfo = $("#wealthProductInfoForm").serialize();
		
		
//		$("#modifyPdCoreInfo_historyEarnrate").val(modifyPdCoreInfo_historyEarnrate);
		//02-固定收益类 01/03-产品子类为浮动/股权类的产品
		if (productSubTypeValue == 02) {
			//固定分类信息
			/*var gdiffrentInfoJson = formDataToJsonObj($("#gdiffrentInfoForm").serialize());
			...
			//浮动/股权分类信息
			/*var fdiffrentInfoJson = formDataToJsonObj($("#fdiffrentInfoForm").serialize());
...
//	var param1 = wealthProductBaseInfo +"&param="+$.toJSON(param);
	...
	if (productTypeValue == '1') {
		...
		if (productSubTypeValue == '02') {
			if (!wealthIncomeDisInfoTableLockOneRow()) {
				$.messager.alert('提示',"请输入固定收益类分类信息必录项！");
				return false;
			}
