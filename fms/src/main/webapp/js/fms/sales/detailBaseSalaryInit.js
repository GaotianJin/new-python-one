 ...
 */
jQuery(function($) {
	month = $("#detailBaseSalary_month").val();
	initDetailBaseSalaryTable();
	});

/**
 * 基本工资列表
 */
var detailBaseSalaryTable;
function initDetailBaseSalaryTable(){
	var  selectIndex = -1;
	detailBaseSalaryTable=$('#detailBaseSalaryTable').datagrid({
		fitColumns:true, 
		rownumbers:true,
		pagination:true,
		singleSelect:true,
		url:contextPath+'/sales/queryBaseSalaryList?param='+month,
		queryParams : {}, // 查询条件
		pageSize:10,
		...
		columns:[[  
		     ...
		     },
		     {
				...
				onLoadSuccess : function() {
					$('#detailBaseSalaryTable').datagrid('clearSelections'); // 一定要加上这一句，要不然datagrid会记住之前的选择状态，删除时会出问题
				},
